...
export const generateProfileQueries = async (
  ...
  const farcasterFid =
    userSocials.find((s) => s.dappName === "farcaster")?.profileName ?? null;
  const lensHandle =
    userSocials
      .find((s) => s.dappName === "lens")
      ?.profileName.replace("lens/", "") ?? null;
  const posts =
    farcasterFid || lensHandle
      ? await getPostsByAuthors(
          [farcasterFid as string, lensHandle as string].filter(Boolean),
          100
        )
      : [];
...
export const getPostsForYou = async (
  ...
  try {
    const alreadyReactedPostIds = await getAlreadyReactedPosts(
      privyAddress,
      500
    );
